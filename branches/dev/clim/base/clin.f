...
C                        OPTION NUMBER 7 -- SETATTR
  800 CONTINUE
      IF (NATTRS .LE. 0) THEN
        CALL ERRGRO (.TRUE.,30)
        WRITE (JOSTND,810) 
  810   FORMAT (T13,'SETATTR KEYWORD MUST FOLLOW CLIMDATA KEYWORD.')
        GOTO 10
      ENDIF
  ...
  820     FORMAT (/1X,A8,'   ATTRIBUTE "',A,'" VALUES CHANGED FROM: ',
     >            4F10.4,' TO: ',4F10.4)
          ATTRS(1:4,I) = ARRAY(2:5)
          GOTO 10
        ENDIF
  ...
  825 CONTINUE
      CALL ERRGRO (.TRUE.,4)
      WRITE (JOSTND,830) 
  830 FORMAT (T13,'SETATTR ATTRIBUTE STRING WAS NOT MATCHED.')
      GOTO 10
      RETURN
      END
