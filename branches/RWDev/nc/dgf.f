...
C----------
      ...
      REAL BRAT,TEMPD1,TEMPD2,BRATIO
      REAL ZRD(MAXPLT)
C----------
...
C  CONSTRAIN RD IF NEEDED
      ...
      IF (DEBUG) WRITE(JOSTND,333) PBA,D,BA,PBAL,RDEN
  333 FORMAT(' DGF PBA,D,BA,PBAL,RDEN =',4F12.3)
C----------
...
C       CONVERT DGLT TO 5-YEAR GROWTH
        DGLT = DGLT/2.0
...
C       MARK CASTLE: DEBUG
     ...
     &  ' BRATIO=', BRAT,' DGLT=', DGLT,' D1=', TEMPD1, ' D2=',TEMPD2,
     &  ' DDS=',DDS
     
C     PROCESS SPECIES OTHER THAN 2, 6, AND 9
      ELSE IF(ISPC .NE. 2 .AND. ISPC .NE. 6 .AND. ISPC .NE. 9) THEN
...
C
C CALCULATE DG FOR SPECIES 2,6, AND 9
      ELSE
...
C  PROCESS REDWOOD
      IF (ISPC .EQ. 12) THEN
        DGCON(ISPC) = -3.384739 + 0.410227*LOG(SITEAR(12))

C  PROCESS SPECIES INDICES THAT ARE NOT 2, 6, 9, O 12
      ELSE IF(ISPC  .NE. 2 .AND. ISPC  .NE. 6 .AND. ISPC  .NE. 9) THEN
