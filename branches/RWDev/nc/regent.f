...
C----------
      ...
      REAL CORTEM(MAXSP)
      INTEGER NUMCAL(MAXSP),K,L,IWHO
      REAL XMAX(MAXSP),XMIN(MAXSP),DIAM(MAXSP)
      REAL P,EDH,XHTGR,SNY,SNX,SSITE,DDS,SCALE,SCALE2,SCALE3
      ...
      CHARACTER SPEC*2
      REAL PBAL,TEMSLP,TEMASP,RDEN,DLO,DHI,SDICS,SDICZ,A,B,SMAX
      REAL D1,D2,DGSM
      REAL ZRD(MAXPLT)
C----------
C  DATA STATEMENTS.
...
C----------
      ...
      END DO
C----------
C ASSIGN TEMSLP AND TEMASP
C USED IN HTG AND DG CALCULATION FOR RW
C----------
      TEMASP = ASPECT
      TEMSLP = SLOPE*100
C----------
...
C  CALCULATE RELATIVE HEIGHT AND TPCCF
      RELHT=1.
...
C  CALCULATE POINT LEVEL BAL
      PBAL=PTBAA(ITRE(I))*(1.0-(PCT(I)/100.))

C  CALL SMALL TREE HEIGHT GROWTH ROUTINE
      HTGR = 0.0
      IF(DEBUG)WRITE(JOSTND,*)'BEFORE HTGR5: I=',I,' ISPC=',ISPC,
     & ' XSITE=', XSITE, ' XBA=', XBA, ' HTGR=', HTGR,
     & ' RELHT=',RELHT, ' CR=', CR, ' H=',H,' PBAL=', PBAL,
     & ' TEMASP=', TEMASP,' TEMSLP=', TEMSLP

      CALL HTGR5(ISPC,XSITE,XBA,HTGR,RELHT,CR,H,PBAL,TEMASP,
     &           TEMSLP)

      IF(DEBUG)WRITE(JOSTND,*)'AFTER HTGR5: I,ISPC,HTGR,CON,XRHGRO=',
     & I,ISPC,HTGR,CON,XRHGRO
...
C     BRANCH FOR RW
C     RW USES DIFFERENT FUNCTIONAL FORM FOR SMALL TREE DG
      ELSE IF(ISPC.EQ.12) THEN

C     CALCULATE SMALL TREE DIAMETER GROWTH
        DGSM = EXP(-3.384739 + 0.225521*LOG(D) - 0.00008*D*D 
     &       - 0.001433*PBAL - 0.346008*RDEN + 0.492242*LOG(CR*10)
     &       + 0.410227*LOG(XSITE) - 0.000502*TEMSLP 
     &       - 0.001519*TEMSLP*COS(TEMASP))
     
C     CONVERT DGSM TO 5-YEAR DG
        DGSM = DGSM/(2.0)
...
C  NOTE: THIS SIMPLIFIES TO IF(IABFLB(ISPC).EQ.1) BUT IS SHOWN IN IT IS
...
C     CALCULATE POINT LEVEL BAL
      PBAL=PTBAA(ITRE(I))*(1.0-(PCT(I)/100.))
      CALL HTGR5(ISPC,XSITE,XBA,XHTGR,RELHT,CR,H,PBAL,TEMASP,
     &           TEMSLP)
