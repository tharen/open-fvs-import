...
C-----------
C CALCULATE RW HEIGHT GROWTH
C REDWOOD USES DIFFERENT FUNCTIONAL FORM
C-----------
      IF(ISPC .EQ. 12) THEN

C SMALL TREE HEIGHT GROWTH CALCULATION
        SMHTG = EXP(-1.614631 - 0.000035*H**2 + 0.325206*LOG(H) 
     &  - 0.001741*PBAL + 0.197669*LOG(CRATIO) + 0.515935*LOG(SINDX)
     &  - 0.001741*SLOPE*100 - 0.001886*(SLOPE*100)*COS(ASPECT))

C  CONVERT TO 5-YEAR HEIGHT GROWTH
        HTG(I) = SMHTG / 2.0
