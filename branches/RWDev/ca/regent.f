...
C----------
      ...
      INTEGER ITRNIN,ISPC,I1,I2,IPCCF,K,L,IWHO
...
C USED IN HTG CALCULATION FOR GS/RW
C----------
      TEMASP = ASPECT
      TEMSLP = SLOPE*100
C----------
...
C
      ...
      ENDIF
C
      CALL SMHTGF(ISPC,D,CR,BA,BAL,SI,H,PBAL,TEMSLP,TEMASP,HTGRR,
     &            RELHT,DEBUG,JOSTND)
C
...
C MARK CASTLE DEBUG
      IF(DEBUG .AND. (ISPC.EQ.23 .OR. ISPC .EQ. 50))WRITE(JOSTND,*)
     & 'AFTER SMHTGF RW/GS',
     & ' H=',H,' PBAL=', PBAL,' CR=',CR,' SI=', SI,' SLOPE=',
     & TEMSLP,' ASPECT=',TEMASP,' ISPC=',ISPC,' HTG=',HTGRR
...
C     RW AND GS USE DIFFERENT FUNCTIONAL FORM FOR SMALL TREE DG
      ELSE IF(ISPC.EQ.23 .OR. ISPC .EQ. 50) THEN
...
C     CALCULATE SMALL TREE DIAMETER GROWTH
     ...
     &       + 0.410227*LOG(SI) - 0.000502*TEMSLP 
...
C     APPLY SMALL TREE DIAMETER GROWTH MULTIPLIER
        DGSM = DGSM * XRDGRO * SCALE
        DG(K) = DGSM 

C     MARK CASTLE DEBUG
        IF(DEBUG)WRITE(JOSTND,*)'IN REGENT ',' D=',D,' PBAL=',PBAL,
     &  ' RDEN=',RDEN,' CR=',CR, ' SI=',SI,' SLP=',TEMSLP,
     &  ' ASP=',TEMASP,' XRDGRO=',XRDGRO, ' BRATIO=', BARK,
     &  ' DG=',DG(K)

C     LOGIC FOR ALL OTHER SPECIES IN CA VARIANT
      ELSE
        BX=HT2(ISPC)
...
C----------
      ...
      CALL SMHTGF(ISPC,D,CR,BA,BAL,SI,H,PBAL,TEMSLP,TEMASP,EDH,RELHT,
     &            DEBUG,JOSTND)
      IF(DEBUG)WRITE(JOSTND,*)'AFTER CALL SMHTGF D,CR,BA,BAL,SI,',
