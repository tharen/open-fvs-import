      ...
      IMPLICIT NONE
C----------
C  **MORTS--WC  DATE OF LAST REVISION:  04/02/14
C----------
...
C  TREE RECORD.  MORTALITY RATE IS PREDICTED FROM DBH, CROWN RATIO,
C  SITE INDEX, AND BASAL AREA INTREES LARGER. IF STAND GETS ABOVE MAX SDI
C  IT WILL APPLY RATE UNTIL STAND IS BELOW MAX. IF THIS ROUTINE
...
C SMC     & 0.01353918  ,0.00991005  ,0.00756365  ,0.00756365  ,0.00756365  ,
     ...
     & 0.01        ,0.0         ,0.0         ,0.005573601 ,0.005573601,
     & 0.005573601 ,0.005573601 ,0.012525642 ,0.009461545 ,0.009461545,
     & 0.009461545 ,0.0         ,0.00991005  ,0.00991005/                     
C 
C-----------
C  CHECK FOR DEBUG.
C-----------
...
C EXCEPT FOR OREGON WHITE OAK WHICH HAS A DIFFERENT EQUATION BUILT 
...
C----------
C     LOOP BACK THROUGH TREES RECORDS IF STAND IF OVER MAXIMUM.
C     BECAUSE A STAND CAN STAY BELOW SDIMAX, BUT BA CAN CREEP UP
C     TO UNREALISTIC VALUES AND ADDITIONAL MAXIMUM FOR BA NEEDS
C     TO BE ADDED (TRADITONAL BAMAX .5454*SDIMAX IS TOO LOW
C     FOR WC AND PN STANDS, AN EQUATION SDIMAX*.65+.25 WAS CREATED
C     USING THE CVS PLOTS AND THE RELATIONSHIP OF SDIMAX AND BAMAX
C     AS AN ADDITIONAL THRESHOLD.
C----------
      IF ((SDIA.LT.SDIMAX).AND.(BAA.LT.550)) GO TO 70
C      IF ((SDIA.LT.SDIMAX).AND.(BAA.LT.SDIMAX*0.65+25)) GO TO 70
C      IF (SDIA.LT.SDIMAX) GO TO 70
      PASS=PASS+1
      SD2SQA=0
      SUMDR10=0   
      TA=0
  59  CONTINUE    
     ...
     &SUMDR10,TA,BAX=',PASS,I,DBH(I),WKI,WKIT2,SD2SQA,SUMDR10,TA,BAX
   60 CONTINUE
      GOTO 55
C----------
