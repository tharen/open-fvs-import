      SUBROUTINE FMCROWLS (SPILS,SPIYV,D,H,IC,SG,XV)
      IMPLICIT NONE

C   **FMCROWLS  FIRE-ON DATE OF LAST REVISION:  01/07/08
...
C     69  jack pine plantation
C     70  white pine plantation
C     71  white spruce plantation
C     72  black spruce plantation                   
...
C  VARIABLE DECLARATIONS
      ...
      INTEGER J, K
      REAL C, ANGLE
      REAL TTOPW, FOL, DBRK(3), TOTABV, BARK, WOOD, BRANCH
      REAL P1, P2, P3, TEMP, F1, F2, F3, F4, HTF, TEMPHT, MYPI, UMBTW(4)
      REAL LILPCE, DIB, DOBF, HTLP, TEMPD, BRATIO
      REAL DX,HX,  DBHCUT, DCTLOW, DCTHGH
      ...
      CALL DBCHK (DEBUG,'FMCROWLS',8,ICYC)
      IF (DEBUG) WRITE(JOSTND,'('' ENTERING FMCROWLS'')')
...
C     RESET D
      ...
      IF ((VVER(1:2) .EQ. 'LS') .OR.
     >   (VVER(1:2) .EQ. 'NE') .OR.
     >   (VVER(1:2) .EQ. 'CS') .OR.
     >   (VVER(1:2) .EQ. 'SN')) THEN 
 
C       SET SOME STUFF UP SO THAT FOR TREES NEAR THE MERCH BREAKPOINT
C       A WEIGHTED AVERAGE OF THE SMALL AND LARGE TREE EQUATIONS ARE USED
C       TO PREDICT TTOPW.
 
        DBHCUT = DBHMIN(SPIYV) + 1.5
        DCTLOW = DBHMIN(SPIYV)
        DCTHGH = DBHMIN(SPIYV) + 3
...
C       SET WEIGHTS BETWEEN LG. AND SM. MODELS FOR TTOPW.
        ...
        TTOPW = (TOTABV - FOL)*SMWGT + BRANCH*(1-SMWGT)
      ENDIF
...
C         (1 = 0-.25, 2 = 0-1, 3 = 0-3, 4 = 0 - 4)
          ...
          DO J = 1,3
            ...
            IF (J .EQ. 1) THEN
              UMBTW(J) = (SG * TEMP / P2T)
            ELSEIF (J .GT. 1 .AND. D .GT. DBRK(J-1)) THEN
              UMBTW(J) = (SG * TEMP / P2T)
            ENDIF
          ENDDO
        ENDIF
