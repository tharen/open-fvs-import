...
C      WRITE(JOSTND,'(T12,"ICYC = ",I5)') ICYC
      IF (ICYC.EQ. 1) THEN
     ...
     >               THISYR,SPCALIB(I),ATTRS(1,I2)
    5     FORMAT ('IN CLMORTS, I=',I3,' SP=',A2,' THISYR= ',F8.2,
     >            ' SPCALIB=',F10.4,' YEARS: ',F10.5)
        ENDDO
      ENDIF
...
C       I IS THE SPECIES INDEX
        
        XV = ALGSLP (THISYR,FLOAT(YEARS),ATTRS(1,I2),NYEARS)  
C        WRITE(JOSTND,'(T12,"THISYR = ",F8.2)') THISYR
C        WRITE(JOSTND,'(T12,"XV = ",F8.5)') XV
C        WRITE (JOSTND,'(T12,"Q01(I) = ",F8.5)') Q01(I)
...
C       PJR CONSIDER P(SURV | 7-YEAR?) AVERAGE FIA REMPER
        ...
        ELSE
          X=0.
        ENDIF
C        WRITE(JOSTND,'(T12,"SPMORT1 = ",F8.5)') SPMORT1(I)
C        WRITE(JOSTND,'(T12,"CLMRTMLT1 = ",F8.5)') CLMRTMLT1(I)
         
C       CONVERT TO A MORTALITY RATE AND APPLY MULTIPLIER.

        FYRMORT(I)=(1.-X)*CLMRTMLT1(I)
C        WRITE(JOSTND,'(T12,"FYRMORT(I) = ",F8.5)') FYRMORT(I)
...
C     COMPUTE THE WEIGHTED AVERAGE MORTALITY RATE FOR CAUSE 2 
      ...
      DO I=1,MAXSP
        ...
        ENDIF
      ENDDO
C      WRITE(JOSTND,'(T12,"LEAVING CLMORTS")')
