      ...
      IMPLICIT NONE
C----------
C    ESIN--STRP   DATE OF LAST REVISION:   06/05/12
C----------
C
...
C
 ...
 1220 FORMAT(/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES= ',A,' (CODE= ',I3,
     &  '); TREES/ACRE=',F6.0,'; % SURVIVAL=',F6.2,/,T13,
...
C     METHOD=1; READ EXTERNAL PROGRAM THAT WILL GENERATE PLANT COMMANDS
 ...
 3900 CONTINUE
C
C     OPTION NUMBER 29 -- REGEN-CALL AN EXTERNAL DLL            REGEN
C                         TO CALCULATE ESTABLISHMENT
      IF(LNOTBK(1)) BARGN  = ARRAY(1)
C
C     READ SUPPLEMENTAL RECORD CONTAINING FULL PATH AND NAME
C     OF KNOWLEGEBASE FILE TO PASS TO REGEN DLL
C
      READ(IREAD,'(A)',END=80)FILRGN
      IRECNT=IRECNT+1
      IF (FILRGN.EQ.' ') THEN
        CALL KEYDMP (JOSTND,IRECNT,KEYWRD,ARRAY,KARD)
        CALL ERRGRO (.TRUE.,4)
        GOTO 10
      ENDIF
        IF(LKECHO)WRITE(JOSTND,3910) KEYWRD,BARGN,FILRGN
 3910   FORMAT(/1X,A8,'   POINT BASAL AREA TRIGGER=',F10.2,/T13,
     &'KNOWLEDGE-BASE FILE= ',A120)
      GOTO 10
C*********************************************************************
C
